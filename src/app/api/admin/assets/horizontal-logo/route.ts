...
export async function POST(request: NextRequest) {
    ...
    return NextResponse.json(
      {
        success: false,
        error:
          "Supabase Storage nao configurado. Defina NEXT_PUBLIC_SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY antes de enviar a logo horizontal.",
      },
      { status: 503 },
    );
  }
  ...
  if (!ALLOWED_MIME_TYPES.has(file.type)) {
    return NextResponse.json(
      {
        success: false,
        error: "Envie a logo horizontal em PNG, JPG, JPEG ou WebP.",
      },
      { status: 400 },
    );
  }
  ...
  if (file.size > 8 * 1024 * 1024) {
    return NextResponse.json(
      {
        success: false,
        error: "A logo horizontal excede o limite de 8 MB.",
      },
      { status: 400 },
    );
  }
  ...
  const filePath = `horizontal-logos/${entitySlug}/${Date.now()}-${safeFileName}`;
...
function sanitizeFileName(fileName: string, fallbackExtension: string) {
  ...
  if (!normalized) {
    return `logo-horizontal.${fallbackExtension}`;
  }
