...
import {
  filterValidLandingImages,
  getValidImageUrl,
  logFilteredLandingImages,
} from "./salon-images";
  ...
  return normalizedServices
    .map((service) => {
      return {
        ...
        title: service.title,
        description:
          preferServiceCopy || clean(stripAutoGeneratedServiceDescription(service.description))
            ? clean(stripAutoGeneratedServiceDescription(service.description)) || undefined
            : undefined,
...
export function getPublicHeroOverlay(salon: Salon): PublicHeroOverlay {
  const title = clean(salon.heroOverlayTitle);
  const subtitle = clean(salon.heroOverlaySubtitle);
...
export function getRealGalleryImages(salon: Salon): SalonGalleryImage[] {
  const images = filterValidLandingImages(salon.gallery);
  logFilteredLandingImages({
    slug: salon.slug,
    section: "gallery-source",
    sourceImages: salon.gallery,
    validImages: images,
  });
  return images;
}
...
function getRealLandingImages(salon: Salon) {
  const sourceImages = salon.realImages?.length ? salon.realImages : salon.gallery;
  const images = filterValidLandingImages(sourceImages);
  logFilteredLandingImages({
    slug: salon.slug,
...
function getAllRealSelectedImages(salon: Salon) {
  const sourceImages = salon.realImages?.length ? salon.realImages : salon.gallery;
  return filterValidLandingImages(sourceImages, { includeLogo: true });
}
...
function getNormalizedPublicImagePlan(salon: Salon) {
  const validImages = getAllRealSelectedImages(salon);

  return normalizeSalonLayoutImagePlan(salon.layoutImagePlan, {
    availableImageIds: validImages.map((image) => image.id),
  });
}
...
export function getPublicHeroImage(salon: Salon) {
  ...
  const landingImages = getRealLandingImages(salon);
  const realHeroImage = landingImages.find(
    (image) =>
      (image.type === "hero" || image.type === "interior"),
  );
  const firstRealImage = landingImages.find((image) => image.type !== "logo");

  return getValidImageUrl(realHeroImage) ?? getValidImageUrl(firstRealImage) ?? "";
}
