...
import { getValidImageUrl } from "./salon-images";
import { normalizeSalonLayoutImagePlan } from "./salon-image-plan";
import { stripAutoGeneratedServiceDescription } from "./service-description-cleanup";
import {
...
export function formDataToInput(formData: FormData): SalonFormInput {
  ...
  const selectedServices = formData
    ...
    .filter(Boolean);
  const observedServices = parseObservedServiceNames(
    getString(formData, "observedServices", ""),
  );
  const allServiceNames = uniqueServiceNames([...selectedServices, ...observedServices]);
  ...
  return {
    ...
    heroOverlayTitle: getOptionalString(formData, "heroOverlayTitle"),
    heroOverlaySubtitle: getOptionalString(formData, "heroOverlaySubtitle"),
    ...
    customDomain: getOptionalStringOrNull(formData, "customDomain"),
    ...
    services: selectedServicesToSalonServices(
      allServiceNames,
      language,
      getServiceDescriptionMap(formData, allServiceNames),
    ),
...
function buildCompleteSalon(partialSalon: Partial<Salon>): Salon {
  ...
  const heroOverlayTitle = clean(partialSalon.heroOverlayTitle);
  const heroOverlaySubtitle = clean(partialSalon.heroOverlaySubtitle);
  ...
  const template = partialSalon.template === "premium_editorial"
    ? "premium_editorial"
    : "default";
  const templateVersion = template === "premium_editorial"
    ? normalizePremiumEditorialVersion(partialSalon.templateVersion)
    : undefined;
  const premiumEditorial = normalizePremiumEditorial(
    partialSalon.premiumEditorial,
    partialSalon,
  );
  ...
  const validGalleryImages = galleryImages.filter((image) => getValidImageUrl(image));
  const availableLayoutImageIds = validGalleryImages.some((image) => image.isReal)
    ? validGalleryImages.filter((image) => image.isReal).map((image) => image.id)
    : validGalleryImages.map((image) => image.id);
  ...
  const realImages = validGalleryImages.filter((image) => image.isReal);
  const hasRealImages = realImages.length > 0;
  const landingReadyImages = validGalleryImages.filter(
    (image) => image.selectedForLanding && image.type !== "logo",
  );
  const selectedHeroImage = validGalleryImages.find(
    (image) =>
      image.isReal &&
  ...
  const heroImage =
    ...
    getValidImageUrl(clean(partialSalon.heroImage)) ||
  ...
  const customDomain = normalizeCustomDomain(partialSalon.customDomain);
...
function dataToPartialSalon(data: SalonFormInput): Partial<Salon> {
  ...
  return {
    ...
    premiumEditorial: data.premiumEditorial,
    heroOverlayTitle: data.heroOverlayTitle,
    heroOverlaySubtitle: data.heroOverlaySubtitle,
...
export function selectedServicesToSalonServices(
  ...
  return normalizedSelectedServices.map((service) => ({
    ...
    title: service,
    description: stripAutoGeneratedServiceDescription(descriptions[service]),
      ...
      .map<SalonService | null>((service) => {
        if (typeof service === "string") {
          ...
          return {
            ...
            title,
            description: stripAutoGeneratedServiceDescription(""),
            price: getServicePriceLabel(language),
        ...
        return {
          ...
          title,
          description: stripAutoGeneratedServiceDescription(service.description),
...
function normalizeGalleryImages(
  ...
  return normalizedImages.map((image, index) => {
    ...
    const src = validUrl || (source === "placeholder" ? fallback.src : "");
    const isReal =
      image.isReal ??
      (source === "instagram" ||
        source === "google" ||
  ...
  return {
    ...
    serviceDescriptions: (copySuggestion.serviceDescriptions ?? [])
      .map((serviceCopy) => ({
        ...serviceCopy,
        description:
          stripAutoGeneratedServiceDescription(serviceCopy.description) ?? "",
      }))
      .filter(
        (serviceCopy) =>
          clean(serviceCopy.title || serviceCopy.originalTitle) &&
          clean(serviceCopy.description),
      ),
...
function getOptionalStringOrNull(formData: FormData, key: string) {
  if (!formData.has(key)) {
    return undefined;
  }

  return clean(formData.get(key)) || null;
}
